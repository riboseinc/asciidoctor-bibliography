...
While the `cite` macro is reserved for CSL styling, all traditional Bib(La)TeX macros are accessible through the same syntax with their usual names: `citet`, `citet*`, `citealt`, `citealt*`, `citep`, `citep*`, `citealp`, `citealp*`, `citeauthor`, `citeauthor*`, `citeyear` and `citeyearpar`. Note that none is missing, since `\cite` is equivalent to `\citet`.
